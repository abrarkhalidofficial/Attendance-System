...
export default function PromptFlowStep3({ setCurrentStep }: { currentStep: number; setCurrentStep: React.Dispatch<React.SetStateAction<number>> }) {
    ...
    <>
      ...
      <Tabs className="px-4 mt-3" defaultValue="voc">
        <TabsList className="bg-[#EBECF6] h-[45px] rounded-lg">
          {['voc', 'vob', 'vom'].map((tab) => (
            <TabsTrigger key={tab} value={tab} className="px-4 rounded-lg transition">
              {tab.toUpperCase()}
            </TabsTrigger>
          ))}
        </TabsList>
          ...
          <div className="flex gap-3">
              ...
              <PopoverContent className="w-80 p-4">
                ...
                <div className="max-h-56 overflow-y-auto space-y-2">
                  {ALL_CONTENT_TYPES.map((type) => {
                    const checkboxId = `voc-type-${type.replace(/\s+/g, '-')}`;
                    return (
                      <div key={type} className="flex items-center space-x-2">
                        <Checkbox id={checkboxId} checked={selectedFilters.includes(type)} onCheckedChange={(checked) => handleCheck(type, !!checked)} />
                        <Label htmlFor={checkboxId} className="font-light text-sm cursor-pointer">
                          {type}
                        </Label>
                      </div>
                    );
            ...
            <Popover open={open2} onOpenChange={setOpen2}>
              ...
              <PopoverContent className="w-80 p-4 space-y-4">
                ...
                <div className="max-h-56 overflow-y-auto space-y-2">
                  {ALL_CONTENT_TYPES.map((type) => {
                    const checkboxId = `vob-type-${type.replace(/\s+/g, '-')}`;
                    return (
                      <div key={type} className="flex items-center space-x-2">
                        <Checkbox id={checkboxId} checked={selectedFiltersBI.includes(type)} onCheckedChange={(checked) => handleCheckBI(type, !!checked)} />
                        <Label htmlFor={checkboxId} className="font-light text-sm cursor-pointer">
                          {type}
                        </Label>
                      </div>
      ...
      <DialogFooter className="mx-4 mt-6">
        <Button className="font-normal" onClick={() => setCurrentStep(4)} variant="outline">
          Cancel
        </Button>
        <Button onClick={() => setCurrentStep(4)} className="font-normal">
          Next
        </Button>
      </DialogFooter>
    </>
