...
export const AttendancePage: React.FC = () => {
  ...
  const { attendanceSessions, clockIn, clockOut, getActiveSession, settings } = useData();
  const [currentTime, setCurrentTime] = useState(new Date());
  const [notes, setNotes] = useState('');
  const [useLocation, setUseLocation] = useState(currentUser?.locationOptIn || false);
  ...
  const handleClockIn = async () => {
    ...
    clockIn(currentUser.id, {
      ...
      faceVerified: false,
      isRemote: !location || settings.officeLocations.length === 0,
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {activeSession ? 'Active Session' : 'Clock In'}
          </CardTitle>
          <CardDescription>
            {activeSession
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span>{format(new Date(session.clockIn), 'MMM d, yyyy')}</span>
                    {session.isRemote && (
                ...
                <div className="text-right">
                  <div>
                    {session.totalHours ? `${session.totalHours.toFixed(2)}h` : elapsedTime}
                  </div>
                  <Badge
